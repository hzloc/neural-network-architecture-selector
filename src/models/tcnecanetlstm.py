...
class ECALayer(nn.Module):
    ...
    def __init__(self, channel, k_size=3):
        ...
        self.conv = nn.Conv1d(
            1, 1, kernel_size=k_size - 1, padding=(k_size - 1) // 2, bias=False
        )
...
class TemporalBlock(nn.Module):
    def __init__(
        ...
        self.downsample = (
            nn.Conv1d(n_inputs, n_outputs, 1, padding=0)
    ...
    def __init__(
        ...
        if dilations is None:
            if dilation_reset is None:
                ...
                dilations = [
                    2 ** (i % dilation_reset) for i in range(len(num_channels))
                ]
...
class TCNETANetLSTM(pl.LightningModule):
    def __init__(
        ...
        self.gru = nn.LSTM(32, 16, 2, batch_first=True)
